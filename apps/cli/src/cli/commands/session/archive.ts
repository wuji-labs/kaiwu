...
export async function cmdSessionArchive(
  ...
  if (!idOrPrefix) {
    throw new Error('Usage: kaiwu session archive <session-id-or-prefix> [--json]');
  }
  ...
  if (await tryHandleApprovalRequestCreated({ envelopeKind: 'session_archive', json, result })) {
    return;
  }

  if (json) {
    await printJsonEnvelope({ ok: true, kind: 'session_archive', data: { sessionId: result.sessionId, archivedAt: result.archivedAt } });
    return;
  }

  console.log(chalk.green('✓'), `已归档 ${result.sessionId}`);
}
