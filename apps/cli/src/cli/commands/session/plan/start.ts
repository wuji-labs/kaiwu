...
export async function cmdSessionPlanStart(
  ...
  if (!idOrPrefix) {
    throw new Error('Usage: kaiwu session plan start <session-id-or-prefix> --backends <id1,id2> --instructions <text> [--json]');
  }
  ...
  if (backendTargetKeys.length === 0 || !instructions.trim()) {
    throw new Error('Usage: kaiwu session plan start <session-id> --backends <id1,id2> --instructions <text> [--json]');
  }
