...
export async function cmdSessionVoiceAgentStart(
  ...
  if (!idOrPrefix) {
    throw new Error('Usage: kaiwu session voice-agent start <session-id-or-prefix> --backends <id1,id2> --instructions <text> [--json]');
  }
  ...
  if (backendTargetKeys.length === 0 || !instructions.trim()) {
    throw new Error('Usage: kaiwu session voice-agent start <session-id> --backends <id1,id2> --instructions <text> [--json]');
  }
  ...
  if (!rawSession) {
    if (json) {
      await printJsonEnvelope({ ok: false, kind: 'session_voice_agent_start', error: { code: 'session_not_found', sessionId } });
      return;
    }
  ...
  if (!started.ok) {
    if (json) {
      await printJsonEnvelope({ ok: false, kind: 'session_voice_agent_start', error: { code: started.errorCode } });
      return;
    }
  ...
  console.log(chalk.green('✓'), '语音智能体已启动');
