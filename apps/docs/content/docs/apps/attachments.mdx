...
Kaiwu lets you attach local files directly to a message in both:
...
Kaiwu supports several attachment flows depending on platform:
...
Before you send the message, Kaiwu keeps attachments as **drafts** in the composer.
...
When you send a message with attachments, Kaiwu uploads each file first and shows **per-file progress** in the composer:
...
Kaiwu keeps the attachment flow provider-agnostic.

When you send a message with attachments:

1. Kaiwu uploads each file first
2. Kaiwu stores the files in the configured attachment upload location
...
If an attachment is an image, Kaiwu shows it inline in the message instead of only listing the filename.
...
Kaiwu does not try to force every image into an inline preview.
...
- Kaiwu falls back to a placeholder/open-file path instead of pretending the preview succeeded

This protects transcript performance while preserving access to the underlying file.

## Attachment storage location

You can control where Kaiwu stores uploaded message attachments.
...
By default, Kaiwu uses this workspace-relative directory:
...
When workspace storage is enabled, Kaiwu can also help keep attachment files out of normal source-control workflows.
...
If a file is over that limit, Kaiwu skips it and tells you why.

### No separate image-count cap

Kaiwu does not add a special “only a few images” composer rule on top of the normal attachment settings.
...
## Attachments together with review comments

Review comments are separate from attachments, but they can be sent in the same prompt flow.

Use them together when you want to hand the agent both:

- a structured set of line-specific review comments, and
- extra local files such as screenshots, logs, or exported artifacts.

In a session, the composer shows review comments as their own **Review** chip/attachment badge when saved comments are available for the current workspace.

In a new-session flow, Kaiwu can also discover saved review comments for the selected machine and directory. If you keep the review-comments chip enabled, the matching review comments are sent with the first actionable message. Turning that chip off does not remove attachment drafts.

## Provider-specific note

Regular session messages support attachments through Kaiwu’s shared attachment flow.
