---
title: Daemon
description: Run Kaiwu in the background (sessions, remote control, headless servers).
---

## What is the daemon?

The Kaiwu **daemon** is a small background process that keeps Kaiwu “available” on your machine.

In practice, it’s what enables Kaiwu to **start and manage sessions even when you don’t have a terminal open**. This is especially important for features like **remote start** (e.g. from the app/web UI): the daemon is the thing on your computer that can receive that request and launch the session locally.
...
Provider login flows in provider settings also depend on the daemon because Kaiwu runs the real provider CLI on the target machine.
