---
title: Updates
description: How updates work across Kaiwu apps, CLIs, installers, Docker images, and release assets.
---

Kaiwu ships multiple apps + tools, and **how updates work depends on how you installed each one**.
...
| --- | --- | --- | --- | --- |
| **Desktop app** (Tauri, “Tori” / Kaiwu desktop) | Installed app bundle (`.app` / installer) | The app checks a signed update feed and offers an in-app update | **Checks automatically**, install is **user-triggered** | Accept the in-app update prompt; stable/preview/dev is determined by which build you installed |
...
| **Managed Relay runtime** (`happier relay host …`) | Install the Kaiwu CLI, then run `happier relay host install …` | You rerun `happier relay host install` | No | `happier relay host install` (local or with `--ssh user@host`) |
| **CLI installer** (`https://kaiwu.chengqiyun.com/install`) | `curl …install \| bash` / `irm …install.ps1` | You rerun it (or use the CLI’s self-update for installer installs) | No | Re-run the installer, or run the lane-specific command (`happier`, `hprev`, or `hdev`) with `self update` |
...
- **minisign**: the checksums file is verified with the embedded Kaiwu minisign public key.
...
There are multiple supported self-host shapes. See also: [Self-host runtime (installer vs runner)](/self-hosting/self-host-runtime).
...
For artifact naming, supported targets, and manifest details, see [Release artifacts](/development/release-artifacts).
