---
title: MCP servers
description: Configure Kaiwu-managed MCP servers, preview the effective tool surface for a session, and reuse the same tools across native-MCP and shell-bridge providers.
---

MCP servers let your agents use external tools — browser automation,
documentation lookup, GitHub APIs, internal systems.

This is Kaiwu *consuming* MCP. If instead you want an editor or automation to
drive Kaiwu itself, that is the other direction:
[Kaiwu MCP (external)](/extending/happier-as-mcp-server).

Kaiwu gives you one place to manage them across the app, daemon, and CLI:

- define MCP servers once in Kaiwu
...
- override Kaiwu-managed MCP selection for a single session without changing your saved defaults

## MCP servers vs Kaiwu tools

MCP servers are one part of the broader **Kaiwu tools** surface.

That means:

- built-in Kaiwu actions and custom MCP tools are configured and delivered as one tool surface
- providers that support **native MCP** receive the tools through MCP directly
- providers that use the **Kaiwu shell bridge** still receive the same Kaiwu-managed tools through the CLI bridge
...
## Kaiwu MCP (external control plane)

In addition to managing *other* MCP servers, Kaiwu can also run **its own MCP server** so external MCP clients can control sessions and execute Kaiwu actions:
...
Shows the MCP servers that you manage directly in Kaiwu.
...
Shows MCP servers Kaiwu found in provider-native config files on the selected machine.

Today, Kaiwu can detect provider-native MCP servers for:

- **Claude**
- **Codex**
- **OpenCode**

Detected servers are shown as **read-only**. You can use this view to understand what the selected backend already has configured, or import those definitions into Kaiwu-managed MCP settings.
...
- “Which Kaiwu-managed servers apply here by default?”
...
When you add a new MCP server in Kaiwu, you can start from three entry points:
...
For local command servers, the main field is a command line so you can paste common README examples directly. Kaiwu then splits that into command + arguments internally.
...
Kaiwu supports common host MCP JSON shapes such as:
...
Kaiwu parses the supported fields, lets you review the result, and then converts it into normal Kaiwu-managed MCP entries.
...
Every Kaiwu-managed MCP server has default scope rules under **Applies to**.

That determines where Kaiwu should include the server automatically.
...
- you can still override Kaiwu-managed MCP selection from the new-session MCP chip

## Secrets, auth, and environment values

Kaiwu lets you set MCP env vars and headers using:
...
Each Kaiwu-managed MCP server can be tested from the MCP settings flow.

Testing verifies that Kaiwu can:
...
- you already configured MCP in Claude, Codex, or OpenCode before using Kaiwu
- you want to compare provider-native config with Kaiwu-managed defaults
- you want to import an existing provider-native server into Kaiwu

Important:

- detected entries are **informational and read-only**
- they are refreshed for the selected machine and directory
- provider-native and Kaiwu-managed MCP can both matter for the final session picture
...
- the built-in Kaiwu tool surface
- Kaiwu-managed MCP servers that apply by default
- provider-native detected MCP servers for the selected backend/context
- Kaiwu-managed servers that are available but not currently applied
...
- see which Kaiwu-managed servers are selected by default
- include additional Kaiwu-managed servers for this session only
- exclude Kaiwu-managed servers that would normally apply by default
...
- the built-in Kaiwu tool surface is always present
...
Kaiwu can currently deliver managed MCP tools through two broad paths:
...
- **Shell-bridge delivery** on backends that use the Kaiwu tools bridge
...
- Kaiwu-managed custom MCP tools can still be delivered through the shell bridge on providers without native MCP delivery
...
Strict mode changes how Kaiwu reacts to invalid MCP configuration.

- **Off**: Kaiwu skips invalid MCP entries when it can and continues starting the session
- **On**: Kaiwu fails the start if an enabled effective MCP server cannot be resolved correctly
...
3. Deselect a default Kaiwu-managed server you do not want
4. Select an additional Kaiwu-managed server you want for this session only
5. Start the session
...
- the new-session MCP chip, which can override Kaiwu-managed defaults per session
