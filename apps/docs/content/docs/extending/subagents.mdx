...
to a review or plan run Kaiwu manages, to teammates inside a Claude team, or
...
- **Kaiwu-managed Subagents**: review, plan, and delegate runs started by Kaiwu
...
- **Kaiwu-managed execution runs and subagents** include review, plan, and delegate runs that Kaiwu can list, route, approve, reopen, and control.
- **Independent Kaiwu sessions** are separate top-level sessions. Creating one is not routine delegation.

Use a Kaiwu-managed run when the user explicitly asks for a Kaiwu-managed run, Kaiwu delegation run, or Kaiwu subagent; asks for another Agent backend, model provider, model, account, or service that the native facility cannot satisfy; or has enabled a custom rule that requires Kaiwu. The presence of a discoverable Kaiwu action does not by itself choose this route.
...
If a provider does not support session modes, Kaiwu simply does not offer the mode control for that session.
...
- latest activity preview, when Kaiwu has loaded that transcript
...
## Agent types in Kaiwu

### Subagents

These are subagents started by Kaiwu itself.
...
Use them when one of the Kaiwu-routing conditions above applies and you want a bounded or long-lived run associated with a Kaiwu session.
...
In Claude sessions, Kaiwu can also surface Claude’s team-style subagents as first-class agents.
...
Some providers emit task/agent sidechains that Kaiwu can normalize into agent rows.
...
For supported agents and runs, Kaiwu keeps transcript access available from both places:
...
- Kaiwu-managed execution runs such as review / plan / delegate,
- Claude teammates surfaced from Claude Agent Teams,
- provider-backed task sidechains that Kaiwu can normalize into agent rows.
...
Use **Launch Subagents** when you want Kaiwu to start a review, plan, or delegate run.
...
Backend selection uses the same current Agent catalog and launch options as the rest of Kaiwu. Inspect the action spec and resolve its options at launch time for the accepted targets, models, configuration, accounts, connected services, and field names. Do not rely on a fixed provider/model recipe from documentation.
...
- Kaiwu opens the **details pane** first
...
Depending on the run type, Kaiwu uses the appropriate execution-run delivery mode under the hood.
...
For Claude team agents, Kaiwu supports routed messages through the session composer.
...
Regular non-routed session messages still support Kaiwu attachments as usual.
...
If the row is a provider-backed monitor-only agent, Kaiwu shows the transcript but does not expose send controls.
...
- Kaiwu can still surface the Kaiwu-subagent launcher
- starting a Subagent resumes the session first, then launches the subagent

That keeps the UI honest: if Kaiwu offers the launcher, the start flow should work.
...
- Kaiwu treats it as **observe-only** for Subagents
- no Kaiwu-subagent launch
...
This is useful when the provider emits side threads but does not expose a supported control surface through Kaiwu.
