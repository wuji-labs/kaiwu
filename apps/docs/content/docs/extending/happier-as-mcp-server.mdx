---
title: Kaiwu MCP (external)
description: Run Kaiwu as an external stdio MCP server so editors and agents can start/manage sessions and execute Kaiwu actions.
---

MCP points in two directions, and this page covers one of them: **something else
driving Kaiwu**. If you are here to give your agent more tools, you want
[MCP servers](/extending/mcp-servers) instead.

Kaiwu can run its own **external MCP server** so other MCP hosts — editors,
agents, automations — can control Kaiwu:
...
- execute any enabled Kaiwu action (using the action spec as source of truth)

This is separate from **MCP servers you manage inside Kaiwu** (Playwright, GitHub, etc). Those are documented under [MCP servers](/extending/mcp-servers).
...
This direct/discoverable exposure model is for Kaiwu action-backed tools. It does not make tools from MCP servers managed inside Kaiwu discoverable-only; those custom tools remain controlled from **Settings → MCP servers**.

## What you are granting

`happier mcp serve` runs with the **same local privileges as the MCP host
process**, using your CLI's authenticated credentials. Configuring a host to
spawn it is a privilege grant, not a connection setting.

That host can execute enabled Kaiwu actions, request approvals — and execute
immediately where the surface does not require one — and reach whatever sessions
your account allows.

<Callout type="warn">
  Treat an MCP host's server configuration the way you would treat a shell
  profile: anything that can edit it can act as you.
</Callout>
...
- The MCP host must run on a machine that is already authenticated to Kaiwu, or the server will fail with `not_authenticated`.
...
Call `execution_run_start` to start a structured Kaiwu-managed run in a session. It requires an `intent` plus a backend target.
...
The backend target selects which provider/backend runs the execution run. It is not a capacity slot. If you choose multiple backend targets through an action that supports that shape, Kaiwu starts one managed run per selected provider/backend.

## What tools you get

The external MCP surface is a blend of:

- **Kaiwu action tools** (generated from the action spec catalog)
...
- If a tool supports inferring the session and you do not pass `sessionId`, Kaiwu uses:
...
Kaiwu treats **actions** as a single catalog that can be enabled/disabled per “surface”, including:
...
That executes the same approval object you can review in the app Inbox. The same generic approval model works regardless of whether the request came from the CLI, an external MCP host, a session agent, or another Kaiwu surface.
...
If you’re targeting a self-hosted server, ensure the MCP host launches Kaiwu with the right server profile (or server URL flags). See [CLI → Server profiles](/apps/cli#server-profiles).

### `No daemon running` / session create fails

Starting and managing local provider processes (creating sessions, sending messages, attaching, etc.) generally requires a running Kaiwu daemon on that machine.
...
- **Settings → MCP servers** (if you expected a *custom MCP server* tool, not a Kaiwu action tool)
