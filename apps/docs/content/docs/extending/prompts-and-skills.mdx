...
description: Manage reusable prompt content in Kaiwu, attach it to agents and profiles, and install or export it to provider-native locations on your machines.
---

Kaiwu groups reusable AI instructions into one product area with a single canonical library and several ways to apply or export what you save there.
...
- **Kaiwu library is the source of truth**
- external files are optional imports, installs, and exports
- Kaiwu tracks explicit links so it can detect drift instead of silently overwriting files
...
## Templates (slash commands inside Kaiwu)

Templates are Kaiwu-native slash shortcuts built from saved prompts.
...
- Kaiwu resolves the linked prompt
- inserts the prompt content into the composer
- optionally sends immediately if the template is configured that way

Kaiwu blocks collisions with built-in action commands, so a template token cannot silently override a core slash action.
...
Kaiwu resolves them into the session’s append/system prompt in a shared way across UI and CLI.
...
- inspect what Kaiwu can read from that machine or project
- import external assets into the library

### What “managed by Kaiwu” means

When you export or install a library item to a machine target, Kaiwu stores an explicit **external link**.
...
This is what lets Kaiwu show whether an external copy is still in sync.
...
For bundle-based targets such as skills, Kaiwu supports:

- **Copy** — writes an independent copy into the selected target
- **Symlink** — writes a Kaiwu-managed materialized copy and links the target to it
...
Kaiwu does not silently overwrite changed external assets.

If a linked external file or directory changes on disk after export:

- Kaiwu detects the digest mismatch
- export, install, or delete actions fail with a conflict instead of overwriting

This is deliberate. The library and the machine copy are related, but Kaiwu still treats local disk mutations as real changes that need review.
...
Kaiwu integrates with the actual public `skills.sh` registry surface instead of only scanning the CLI repo.
...
Kaiwu scans those sources for skill bundles and exposes them through the same registry UI as built-in sources.
...
These terms mean different things in Kaiwu:

### Import

Bring something **into the Kaiwu library**.
...
- explicit external link recorded by Kaiwu
...
Kaiwu keeps the library model provider-agnostic, but external asset support depends on each provider’s file conventions.
...
Kaiwu is designed so you can reason about prompt content from one place first, then connect it to machines and providers as needed.

### External files are real

If you or another tool edits an exported asset on disk, Kaiwu treats that as a real external change and refuses silent overwrite.
