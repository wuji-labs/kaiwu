...
This guide covers GitHub auth on the Kaiwu server:
...
Note: Kaiwu accounts are still **device-key** accounts. GitHub acts as an attached identity provider (used for signup, connect, and policy enforcement), not as a replacement for device-key login.
...
Kaiwu uses a **GitHub OAuth App** (not a GitHub App) for the OAuth redirect flow.
...
Notes:
- The Kaiwu server currently talks to `github.com` / `api.github.com` directly, so this setup is for **GitHub.com** (not GitHub Enterprise Server).
...
3. Open the Kaiwu app and confirm “Continue with GitHub” is available.
4. If your GitHub login is already taken (or doesn’t match the username rules), Kaiwu will ask you to choose a username during signup. This is expected.
   - Important: this is **signup** (creating a new Kaiwu account). If you’re trying to use GitHub to “log in” to an existing Kaiwu account from another device, you must use **device-key restore** (for example **Add your phone** / **Restore account**) and then connect GitHub from **Settings → Account**.

Troubleshooting:
- If you see an error like “provider already linked”, it means this GitHub identity is already connected to an existing Kaiwu account on that server.
  - Use **device-key restore** on this device/browser, then continue.

## Keyless GitHub login (optional, enterprise)

Kaiwu can also use GitHub OAuth as a **keyless login method** (no device keys).
...
- On `HAPPIER_FEATURE_ENCRYPTION__STORAGE_POLICY=optional`, the client may offer a **choice** on first login (E2EE vs plaintext) when GitHub is enabled for both keyed provisioning and keyless auto-provisioning. See [Server Auth](/self-hosting/auth).
...
Required **unless** you are using the hosted client at `https://kaiwu.chengqiyun.com`:
- `HAPPIER_WEBAPP_URL` (default `https://kaiwu.chengqiyun.com`)
...
- Default return base: `HAPPIER_WEBAPP_URL` (`https://kaiwu.chengqiyun.com`)
...
Troubleshooting:
- If you get redirected to `https://kaiwu.chengqiyun.com` even after setting `HAPPIER_WEBAPP_URL`, the server likely rejected your configured return URL as unsafe. For local web dev, use `http://localhost:PORT` or `http://*.localhost:PORT` (or use `https://`).
