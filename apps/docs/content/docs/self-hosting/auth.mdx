...
- [GitHub auth](/self-hosting/auth-github)
- [OIDC auth](/self-hosting/auth-oidc)
- [mTLS auth](/self-hosting/auth-mtls)
- [Custom auth providers](/self-hosting/auth-custom-providers)
...
- GitHub: [GitHub auth](/self-hosting/auth-github)
- OIDC: [OIDC auth](/self-hosting/auth-oidc)

## Standard authentication (device-key accounts)

Kaiwu uses a **device-key** account model (each account is tied to a device secret).
...
- External providers are **not** “password login”: if a user is trying to access an *existing* Kaiwu account from a new device/browser, they still need to **restore** the account (for example via **Add your phone** / **Restore account**), then connect providers from settings.
...
- **Keyed provisioning** (“Continue with GitHub”): creates a **device-key** account.
  ...
  - This is the standard Kaiwu model and works on E2EE-only servers.
...
Kaiwu advertises auth entrypoints via:
...
- `keyed`: device-key account (default Kaiwu model)
...
Kaiwu currently supports keyless login via:
...
- for mTLS keyless login: see [mTLS auth](/self-hosting/auth-mtls)
...
- If you use the hosted web app, you don’t need to set anything (default is `https://kaiwu.chengqiyun.com`).
...
See [GitHub auth](/self-hosting/auth-github) for full setup steps.

### OIDC-only signup (example: Okta) + OIDC required for login

See [OIDC auth](/self-hosting/auth-oidc) for provider configuration.
