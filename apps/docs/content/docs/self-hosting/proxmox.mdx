---
title: Proxmox VE
description: Deploy Kaiwu Server on Proxmox VE using community-scripts helper scripts.
---

This guide explains how to deploy Kaiwu on **Proxmox VE** using the community-maintained Proxmox VE Helper-Scripts project (LXC-based “one command” installs).

## What you get

- A dedicated **Debian LXC container** running Kaiwu Server with PGlite embedded database.
...
1. In a browser, open `helper-scripts.com` and search for **Kaiwu**.
...
Kaiwu has two separate “things to connect”:
...
If you only access Kaiwu from your LAN, plain HTTP can work.
...
- Open **Settings → Relays** and add your relay URL.

If you chose **not** to serve the web UI from your Proxmox container, use:

- the **mobile app** (recommended), or
- the hosted web app at `https://kaiwu.chengqiyun.com` and set the server there
...
## Using Kaiwu from another machine (common setup)
...
For ongoing profile switching and multi-server usage, see [Features → Multi-server](/accounts/multi-server).
...
By default, the Proxmox helper script installs Kaiwu using the managed **Relay runtime** (`happier relay host` under the hood), which supports stable/preview/dev channels and in-place updates.
...
For remote usage, the Kaiwu web UI needs **HTTPS** (secure context). Typical setups:
...
- Use the Proxmox helper script “Update” action (it runs `happier relay host install` and also updates the Kaiwu CLI if installed).
