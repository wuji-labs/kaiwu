...
description: Use third-party routers, switchers, and plugins with Kaiwu (Claude Code + OpenCode).
---

Kaiwu runs **provider CLIs** (for example: **Claude Code** and **OpenCode**) as subprocesses, then syncs transcripts/state to your Kaiwu server so you can continue from other devices.
...
To use them with Kaiwu, the only requirement is: **those settings must be visible to the provider CLI process that Kaiwu spawns**.

This matters both for normal provider sessions and for the **Authentication** section inside each provider settings page, because Kaiwu reads the same machine-local provider auth state from the CLI environment, local files, or provider-native status commands.

For user-facing MCP management inside Kaiwu itself, including Kaiwu-managed MCP servers, detected provider-native MCP configs, preview, JSON import, and per-session MCP selection, see:
...
If you start a session from your terminal, Kaiwu inherits your shell environment and passes it through to the provider process.
...
Avoid `ccr code …` for Kaiwu usage: it launches Claude Code directly, which bypasses Kaiwu’s session syncing.
...
2) Switch providers in your shell (example):
   - `eval "$(ccm glm global)"`
3) Start Kaiwu normally:
   - `happier`

Do **not** use `ccc …` with Kaiwu. `ccc` launches `claude` directly (bypassing Kaiwu). You want Kaiwu to be the process that spawns `claude`, with the switched env vars already set.
...
If you prefer `claude-code-switch` to write `~/.claude/settings.json` (user-level) or `.claude/settings.local.json` (project-level), those can also work with Kaiwu because the provider CLI will read those files when it starts.
...
`oh-my-opencode` is primarily an **OpenCode plugin/config installer**. Kaiwu runs OpenCode via ACP (`opencode acp`), so the general rule is:

- If `opencode` uses the plugin when you run it directly, it should behave the same when Kaiwu runs it.

Recommended usage with Kaiwu:

1) Install OpenCode.
2) Install/configure `oh-my-opencode` (it typically updates your OpenCode config under `~/.config/opencode/…`).
3) Start an OpenCode session in Kaiwu (OpenCode mode).
