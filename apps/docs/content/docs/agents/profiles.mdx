...
Kaiwu ships with built-in profiles for common setups, plus your own custom profiles.
...
5. Add the environment variables Kaiwu should inject when the session starts
6. Mark any required values as:
   - **Secret** when Kaiwu should satisfy them from a saved secret, machine env, or prompt flow
...
3. Pick a profile
4. Complete any missing requirements if Kaiwu asks

For UI-started or remote-started sessions, Kaiwu can satisfy required secrets from:
...
Configured ACP backends participate in this flow the same way built-in backends do. If a profile is intended for one specific backend, Kaiwu preserves that relationship directly.
...
- otherwise Kaiwu matches by **case-insensitive name**
- if more than one profile has the same name, Kaiwu asks you to use the id
...
2. saved secret binding from your Kaiwu settings
...
Kaiwu will apply the required auth token and the Claude-compatible env mapping for you.
...
- the profile is saved in the same Kaiwu account/server you are targeting
...
- rerun the command in an interactive terminal so Kaiwu can prompt for it
