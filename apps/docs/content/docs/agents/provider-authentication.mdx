...
description: Manage machine-local provider CLI authentication from Kaiwu, check auth status, and open provider login flows inside the app.
---

Kaiwu can inspect and manage the **machine-local authentication state** of supported provider CLIs directly from each provider settings page.
...
- **Connected services** = credentials stored in Kaiwu and reused across compatible backends
...
The Authentication section is machine-specific. If you switch to a different machine, Kaiwu checks that machine’s provider CLI instead.

## What Kaiwu shows

Depending on the provider, Kaiwu can show:
...
- **Last checked** — when Kaiwu last refreshed that auth state

## Actions

### Log in / Re-authenticate

If the provider supports a native CLI login flow, Kaiwu can open it directly inside the app.

What happens:

- on larger layouts, the login terminal opens in the same shared bottom-pane system used by session terminals
- on phone-sized layouts, Kaiwu uses a more focused terminal presentation
- Kaiwu launches the provider’s real CLI on the selected machine
- if the provider needs an in-terminal command such as `/login`, `/auth`, `/setup`, or `/connect`, Kaiwu sends it for you
- if the provider prints a URL, Kaiwu shows copy/open actions in the terminal UI

When the login flow exits, Kaiwu refreshes the provider auth state automatically.
...
- you logged in outside Kaiwu and want the status to update immediately
...
If Kaiwu shows **Unknown**, that usually means one of these:

- the provider does not expose a reliable non-interactive auth probe yet
- the provider CLI check timed out or failed
- Kaiwu intentionally avoided an interactive provider command during background detection

Unknown does **not** automatically mean “logged out”.

## Which providers support what

| Provider | Login from Kaiwu | Auth status detection |
| --- | --- | --- |
| Claude | Yes | Yes |
| Codex | Yes | Yes |
| Gemini | Yes | Yes |
...
| Pi | No native login flow in Kaiwu | Yes, env-based |

“Limited / conservative” means Kaiwu can launch the login flow, but only reports a positive auth state when the provider exposes a safe local signal.
...
See [Custom ACP](/agents/custom-acp).

## How the login terminal fits with the rest of Kaiwu

The provider login terminal is not a one-off modal. It reuses the same terminal foundation as Kaiwu’s embedded session terminal:
...
### “I already logged in in my terminal and want Kaiwu to pick it up”

Open the provider’s settings page and use **Check now**.

### “I want to know which account the machine is using”

If the provider exposes an account label safely, Kaiwu shows it in **Logged in as**.

### “I use env-based auth instead of interactive login”

Kaiwu can detect env-backed auth for providers that expose it and shows the auth method accordingly.
...
- If a provider login flow changes upstream, Kaiwu follows the provider CLI’s real current behavior rather than inventing a custom auth flow.
...
If the provider/backend supports Kaiwu [Connected services](/accounts/connected-services), that is often the easiest way to make the target machine ready for handoff without depending only on a native CLI login on that machine.
