...
If you want to validate hstack or a Kaiwu PR using `hstack review-pr` on a **fresh isolated system** (no existing `~/.happier-stack`, no host tooling), the simplest repeatable approach on Apple Silicon is a Linux VM managed by **Lima** (it uses Apple’s Virtualization.framework).
...
When you want to open Kaiwu/Expo URLs in your macOS browser, **use localhost port forwarding**.
