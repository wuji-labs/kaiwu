...
description: Use HTTPS secure context on phones/laptops via Tailscale Serve.
---

Kaiwu relies on “secure context” browser features (WebCrypto). Browsers treat `http://localhost` as a secure context, but **not** `http://<lan-ip>:<port>` or `http://<tailscale-ip>:<port>`.
...
The Kaiwu mobile app has an “API Endpoint” (or “Custom server URL”) setting in developer mode.
