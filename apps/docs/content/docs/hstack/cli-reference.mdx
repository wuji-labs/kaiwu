...
Stack-scoped app CLI (runs the Kaiwu CLI/daemon in stack context):
...
Provision a remote machine over SSH (install Kaiwu, pair credentials, optionally install/start the daemon service):
