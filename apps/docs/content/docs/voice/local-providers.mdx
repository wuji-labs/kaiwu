...
description: Configure Local voice in Kaiwu with device, self-hosted, Google, and local-neural STT/TTS providers.
---

**Local voice** is Kaiwu’s configurable speech pipeline.
...
TTS can still stay enabled here, so Kaiwu can read replies back to you.
...
- **Daemon voice agent**: uses Kaiwu’s daemon-backed voice agent runtime
- **OpenAI-compatible voice agent**: calls your configured chat-completions endpoint directly

Use the daemon backend when you want the tightest Kaiwu integration, including the working-directory and teleport behavior described below.
...
- Kaiwu resolves a machine automatically when it starts the voice agent
...
When you start voice directly from a session, Kaiwu can start the voice agent in that session’s project root.
...
Provider resume is capability-driven. When it is not available for the current backend/agent combination, Kaiwu disables it instead of pretending it will work.
...
When a backend exposes a dynamic model list, Kaiwu uses that live list in the dropdown instead of forcing manual text entry.
...
If enabled, Kaiwu speaks replies automatically after a turn completes.
...
Use **Network timeout** to control how long Kaiwu waits for STT or TTS network operations before failing.
...
When **Device STT** is selected, Kaiwu can also expose **hands-free** controls such as:
...
Kaiwu’s **Local neural TTS** currently uses **Kokoro**.
...
Both Kokoro native TTS packs and Sherpa native STT packs are downloaded on demand from Kaiwu’s model-pack manifests.
