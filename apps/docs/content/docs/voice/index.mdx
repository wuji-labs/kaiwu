---
title: Voice
description: Choose a voice mode, understand how voice conversations map to sessions, and configure ElevenLabs or local voice in Kaiwu.
---

Kaiwu supports four voice modes in **Settings → Voice**:

- **Off**: disables all voice features.
- **Kaiwu Voice**: realtime ElevenLabs voice using Kaiwu-managed credentials and server-side quotas.
...
- choose **Kaiwu Voice** for the simplest “just works” realtime voice experience
...
**Preferred language** sets the language Kaiwu should prefer for spoken responses and voice-side interactions.
...
Kaiwu includes privacy controls for what voice providers can see.
...
Voice in Kaiwu is more than a floating microphone button. It has its own conversation state and can act on real sessions.

### Hidden voice conversation

When voice is active, Kaiwu keeps a hidden voice conversation session.
...
**Kaiwu Voice** is the managed realtime option.
...
Kaiwu Voice is available only when the connected server advertises it through `/v1/features`.
...
When creating a restricted ElevenLabs API key for Kaiwu, enable:
...
Your ElevenLabs API key is saved in Kaiwu as an encrypted secret setting rather than a plaintext field, so you do not need to re-enter it every time.

### Auto-provisioning a Kaiwu agent

If you do not want to create or update the ElevenLabs agent manually, Kaiwu can help provision it for you.

From the ElevenLabs section you can:

- create a new Kaiwu-compatible agent
- update an existing Kaiwu agent template
- reuse an existing agent if you already have one

This is the easiest way to keep your ElevenLabs agent aligned with Kaiwu’s current tool and prompt wiring.
...
When you use **Agent** mode, Kaiwu does not guess a fake model list locally.

- the **Voice agent backend** list follows your enabled backend toggles
- the **Agent machine** setting decides which machine Kaiwu probes for backend capabilities
...
If the selected machine cannot provide a dynamic list for the chosen backend, Kaiwu falls back to the safe options that are always valid:
...
Even in **Direct to session**, TTS can still be enabled so Kaiwu can read agent replies back to you.

### Hands-free mode

When you use **Device STT**, Kaiwu can also expose hands-free endpointing controls such as silence timeout and minimum speech duration.
...
Voice uses the same structured action system as the rest of Kaiwu.
