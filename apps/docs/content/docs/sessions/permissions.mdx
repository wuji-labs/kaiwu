...
Permissions determine **how often Kaiwu asks for approval** when an agent wants to run tools (like editing files or running commands), and how restrictive the session environment should be.
...
Kaiwu stores your choice in the session so it stays consistent when you open the same session on another device.
...
Not every provider supports every mode. When a mode isn’t supported, Kaiwu will **fall back** to the closest supported behavior and show you the **Effective** policy in the UI.

Legacy note:

- some older sessions and compatibility paths may still carry `plan` in permission metadata
- in current Kaiwu builds, **Plan** is treated as a **session mode**, not a normal permission choice
- when Kaiwu sees legacy `permissionMode=plan`, it maps that to **Mode = Plan** for compatible providers
...
When session modes are available, Kaiwu also supports returning to the provider default mode. In the app this appears as the default/no-override option.
...
- when you exit plan mode, Kaiwu returns the session to the normal working mode for that provider

### Exiting plan mode

When a provider asks to exit plan mode, Kaiwu surfaces that as a **user-action request**, not as a generic permission prompt.
...
Resume is now a **session-level eligibility decision** in Kaiwu, not a normal runtime-probing flow.

In practice, Kaiwu decides resumability from:
...
- **App Server** is the default resumable path in Kaiwu
- if Codex is set to plain `mcp`, fresh sessions can still run there, but vendor resume is not available on that path
- explicit resume remains **fail-closed**: Kaiwu errors instead of silently starting a fresh session
...
For ACP providers that advertise supported models, Kaiwu will show the provider’s live list.
For providers without a live list (like Claude Code), Kaiwu may allow entering a **custom model id**.
...
- **Codex/OpenCode/Kilo/Auggie (ACP)**: Kaiwu applies model changes live when `session/set_model` is supported; otherwise it falls back to provider config options when available.
- **Gemini (ACP)**: model changes may require recreating the underlying ACP process; Kaiwu preserves context using replay/history so the session still continues.
...
The CLI accepts common aliases and resolves each to a canonical mode:
...
`auto` is the row worth reading twice. It resolves to `safe-yolo`, which lets the
agent write inside the workspace without asking each time. If what you wanted was
"ask when needed", pass `ask` or `normal`.

There is a second step after this one. An agent that has no distinct mode for a
canonical value narrows it further when the session starts: both Claude and the
Codex-like agents fold `acceptEdits` into `safe-yolo` and `bypassPermissions`
into `yolo`, and Claude additionally treats `plan` as read-only. So the table
above is what the CLI parses, and what your agent actually runs is that value
narrowed to the modes it supports.

If you pass an unknown value, Kaiwu prints the supported list and examples.
...
Use **Settings → Actions** when you want to choose, per action and per surface, whether Kaiwu should:
...
For the broader model, see [Kaiwu tools & actions](/extending/happier-tools) and [Inbox and approvals](/sessions/inbox-and-approvals).
...
When Kaiwu shows a permission prompt, some providers can also offer scoped “don’t ask again” choices.
...
- if the provider/runtime does not support a matching allowlist update, Kaiwu falls back to normal prompting

### Push notifications for permission and user-action requests

When push notifications are enabled for the account, Kaiwu can also notify you when a session is blocked on either:
...
If the notification targets a server that is not already trusted on the device, Kaiwu routes you through the safer add/switch flow instead of auto-approving on an unknown server.
...
- **App Server**: the default backend in Kaiwu and the normal resume path when the session has valid Codex resume metadata
- **ACP**: still available when you explicitly switch Codex to ACP mode; resume can also work there
- **MCP**: still usable for normal fresh sessions, but plain MCP is not a vendor-resume path in current Kaiwu builds

In Kaiwu, **permissions remain the primary control** for Codex. Kaiwu maps your selected permission intent to the closest backend-native policy for the active Codex backend.
...
When you allow a shell command and ask Kaiwu to remember it, the grant is
...
approved as a unit. Otherwise Kaiwu parses the command and applies three
rules:
