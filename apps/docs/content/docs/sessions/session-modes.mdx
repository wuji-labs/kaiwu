---
title: Session modes
description: Understand provider-defined session modes such as Plan, how they differ from permissions, and how Kaiwu handles plan approval flows.
---
...
Kaiwu only shows the **Mode** control when the active provider/backend supports session modes for that session.
...
- **Permission mode** changes how Kaiwu and the provider handle approvals
...
Plan mode is the most important session-mode example in current Kaiwu builds.

When a session starts or switches into **Plan**:

- the provider is expected to reason about the work and propose a plan
- Kaiwu keeps the session in plan mode until the provider asks to exit it
...
When the provider is ready to leave planning and begin execution, Kaiwu surfaces an **Exit Plan Mode** user-action request.
...
- Kaiwu clears the session’s plan-mode override
...
If you choose **Request changes**, Kaiwu keeps the session in plan mode and sends the revision request back to the provider.
...
Kaiwu keeps plan review separate from generic permission requests so it is easier to understand what decision is actually being requested.
