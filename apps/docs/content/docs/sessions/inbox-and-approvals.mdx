...
Instead of executing directly, Kaiwu can create an approval request that you review first.
...
In **Settings → Actions**, Kaiwu stores this as per-action, per-surface policy.

That means approvals are not implemented as one-off action-specific code paths.
The same generic approval flow can be reused across:
...
Kaiwu also surfaces **user-action requests**, where the agent needs a structured human response before it can continue.
...
Depending on the request, Kaiwu can ask you to:
...
2. Kaiwu shows an **Exit Plan Mode** request
3. you can:
   - **Approve Plan**
   - **Request changes**
...
This distinction is important in current Kaiwu builds:
...
An approval request is about **running a higher-level action through Kaiwu’s action system**.
...
2. Kaiwu creates an approval request
3. the request appears in **Inbox**
4. you open it and review the summary/details
5. you approve or reject it
6. if approved, Kaiwu executes the underlying action and records the result
...
4. If Kaiwu offers a scoped “don’t ask again” choice, use that when you want the rest of the session to stop prompting for the same tool or command pattern
...
4. Kaiwu sends that response back through the generalized user-action response path
...
- [Prompts, skills, templates, and registries](/extending/prompts-and-skills)
