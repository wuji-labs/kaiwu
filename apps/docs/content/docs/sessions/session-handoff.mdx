...
description: Move a live Kaiwu session between machines, keep the same session id, and understand workspace-transfer and direct-session options.
---

Session handoff lets you move a live Kaiwu session from one machine to another without forking it.

The important promise is simple:

- the same Kaiwu session id stays in place
- the active owner changes from one machine to another
- the conversation can continue on the target machine

This is different from:

- [Session forking & replay](/sessions/session-forking), which creates a branch
- [Attach to a running session](/sessions/attach-to-session), which reopens a session from a terminal on the same machine
- [Continuing a session](/sessions/continuing-a-session), which explains where the transcript lives
- [Continue with another Agent](/sessions/continue-with-another-agent), which changes the coding Agent in place on the machine that already hosts the session
...
- the Kaiwu session id stays the same
...
See [Continuing a session](/sessions/continuing-a-session).

### Kaiwu sessions

For a Kaiwu session, handoff moves the live owner to another machine while keeping the same Kaiwu-managed session.
...
Use Convert to synced when you want to move out of the machine-bound direct model and let Kaiwu own the session going forward.

## Workspace transfer

Workspace transfer is optional.

By default, Kaiwu keeps the target workspace unchanged unless you turn transfer on.
...
Kaiwu blocks workspace transfer for unsafe source paths such as:
...
- the credential lives in your Kaiwu account
...
- Connected services help when the backend can materialize auth from Kaiwu-managed credentials
...
When you start handoff, Kaiwu shows a progress modal while it:
...
- the session stays in place in Kaiwu
...
- move from one dedicated Kaiwu home to another
...
### Convert a direct session into a Kaiwu session

Use Convert to synced when you started from a provider-native session but now want Kaiwu to own the transcript and future continuity.
...
- Kaiwu detected an unsafe source path such as `~` or a filesystem root
