...
description: Understand how Kaiwu updates a busy lead session or a running execution run without losing your message.
---

Kaiwu supports two related busy-send behaviors:
...
When the current backend/runtime supports in-flight steering, Kaiwu can inject the message into the active turn instead of forcing you to wait.

If steering is not available for that session/runtime, Kaiwu falls back to the normal busy-send path for the main session.
...
When the selected recipient is a **running execution run**, Kaiwu shows a **Delivery** chip next to the composer.
...
- if the backend supports steering, Kaiwu steers the active turn,
- otherwise Kaiwu aborts the current turn and immediately sends a new turn,
