---
title: Bug reports
description: Send issue reports from Kaiwu with optional diagnostics for faster triage.
---

Kaiwu includes a built-in bug report flow from app settings.
...
Use these env vars on the Kaiwu server:
...
`HAPPIER_SERVER_OWNER_USER_IDS` uses Kaiwu account ids (not usernames). Owners can copy their id from Settings → Account in the app.
...
To reduce duplicate reports, Kaiwu can search GitHub for potentially similar issues while you fill the report.
...
If you want maintainers to be able to reach you, provide your GitHub username via `--github-username`. Kaiwu appends a `Reporter GitHub: \`...\`` line to the report summary (sanitized to avoid accidental formatting/pings).
