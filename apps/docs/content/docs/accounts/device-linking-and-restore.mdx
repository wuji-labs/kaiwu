...
Kaiwu now has a few different QR-based flows. They look similar, but they solve different problems.
...
- **Account restore / add a phone** signs a new device into the same Kaiwu account
...
- you want your phone to sign in to that same Kaiwu account
...
When your phone scans it, the computer stays in control of approval. If Kaiwu shows a confirmation code on both devices, check that they match before approving.
...
- **Phone / mobile-sized web with camera support:** Kaiwu opens the QR scanner first so you can scan a desktop/web QR immediately
- **Desktop / larger web layouts:** Kaiwu shows a QR code that another already-signed-in device can scan
...
Use **Scan QR to connect terminal** only when a terminal or CLI is showing a Kaiwu terminal QR.
...
- you copied a Kaiwu link from another device
...
4. Check the confirmation code if Kaiwu shows one
...
If you are using a phone as the new device, Kaiwu usually opens the scanner first instead.

### Connect a terminal

1. Start terminal login from the CLI
2. In the Kaiwu app, choose **Scan QR to connect terminal**
...
When Kaiwu asks an already-signed-in device to approve a restore or pairing request, treat it like a login confirmation.

Recommended checks:

- confirm that the request appears when you expect it
- check the short confirmation code if both devices show one
- verify the device label when Kaiwu shows it
...
To avoid creating broken mobile links, Kaiwu intentionally omits the server URL from mobile-oriented QR codes and deep links when the current server is loopback-only.
...
If you operate the server yourself, set `HAPPIER_PUBLIC_SERVER_URL` so Kaiwu can embed the right shareable URL in QR/deep links.
