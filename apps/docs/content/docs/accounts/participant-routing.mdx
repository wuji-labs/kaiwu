...
Kaiwu supports **participant routing** inside a session.
...
Kaiwu remembers the selection **per screen while that screen stays open**. If you leave and reopen the screen, the selection resets to the default state for that screen.

## View-aware automatic selection

If you open a focused teammate or run transcript, Kaiwu can auto-select that recipient for the composer.
...
- Kaiwu keeps your original text in the transcript,
...
- Kaiwu rewrites the backend prompt that Claude receives,
...
When the selected recipient is an execution run, Kaiwu shows a **Delivery** chip.
...
- if the backend supports in-flight steering, Kaiwu steers the current turn immediately,
- otherwise Kaiwu aborts the current turn and sends your message as the next turn,
- the run itself keeps going; Kaiwu does **not** kill the whole run just because steering is unavailable.
...
- Kaiwu always aborts the current turn,
...
This is why a run can sometimes still say **running** in a broad sense but no longer offer a send control: Kaiwu follows the runtime’s real sendability contract instead of guessing from a stale transcript state.
