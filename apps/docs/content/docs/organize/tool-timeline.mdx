...
description: How Kaiwu renders tools consistently across providers, with configurable detail levels.
---

Kaiwu normalizes provider-native tools into a canonical timeline surface (for example `Bash`, `Edit`, `Read`, `Diff`).
...
Depending on provider/runtime payload quality, Kaiwu can surface:
...
If you want the live roster for those agents, use the dedicated [Agents & subagents](/extending/subagents) surface. The transcript keeps the launch history; the Agents tab keeps the current running/recent view.
