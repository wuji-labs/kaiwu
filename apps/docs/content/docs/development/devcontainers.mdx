---
title: Dev box
description: Run Kaiwu CLI + daemon inside a container (dev-box), and pair it to your account without opening a browser.
---

This page explains how to run Kaiwu inside a container (for example: a remote VM or a disposable “dev box”) and authenticate it **from your local machine**.
...
The image does not include a source checkout or `hstack`; it is meant for using Kaiwu from a container, not developing Kaiwu itself.
...
Set `HAPPIER_PROVIDER_CLIS` to install provider CLIs through the Kaiwu CLI:
...
## Recommended: persist Kaiwu state
