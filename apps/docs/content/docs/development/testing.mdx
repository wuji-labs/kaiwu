...
Kaiwu uses the native TypeScript 7 compiler for first-party typecheck and package-build lanes. The `typescript` package remains on the API-compatible 5.9 line for AST-based tools and integrations that do not yet support TypeScript 7's native API.
...
Kaiwu includes a small opt-in probe that runs your locally installed `claude` CLI to discover the exact Agent Teams tool names and capture representative payload shapes:
