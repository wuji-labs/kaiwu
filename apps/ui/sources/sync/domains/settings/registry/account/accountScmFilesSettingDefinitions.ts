...
export const ACCOUNT_SCM_FILES_SETTING_DEFINITIONS = defineSettingDefinitions({
    ...
    filesDiffReviewCommentsInlineVirtualizationLineThreshold: {
        schema: z.number(),
        default: 120,
        description: 'Line threshold for enabling inline diff virtualization in review comment diffs',
        storageScope: 'account',
        analytics: { trackCurrentState: true, trackChanges: true, valueKind: 'bucket', privacy: 'bucketed', identityScope: 'person', serializeCurrent: serializeCountBucket(120, 240) },
    },
    ...
    filesImagePreviewCacheMaxEntries: {
        ...
        storageScope: 'account',
        analytics: { trackCurrentState: true, trackChanges: true, valueKind: 'bucket', privacy: 'bucketed', identityScope: 'person', serializeCurrent: serializeCountBucket(32, 64) },
    },
    filesImagePreviewCacheMaxTotalBytes: {
        schema: z.number(),
        default: 128 * 1024 * 1024,
        description: 'Maximum total bytes retained by cached image previews (best-effort; Blob/file-backed previews use raw file bytes)',
        storageScope: 'account',
        analytics: { trackCurrentState: true, trackChanges: true, valueKind: 'bucket', privacy: 'bucketed', identityScope: 'person', serializeCurrent: serializeBytesBucket(96 * 1024 * 1024, 128 * 1024 * 1024) },
    },
    filesImagePreviewMaxBytes: {
        schema: z.number(),
        default: 16 * 1024 * 1024,
        description: 'Maximum raw file bytes for a single in-app image preview',
        storageScope: 'account',
    ...
    markdownDefaultEditMode: {
        schema: z.enum(['raw', 'rich']),
        default: 'rich',
        description: 'Default editing mode for markdown files in the embedded file editor: raw text or rich (WYSIWYG)',
        storageScope: 'account',
        analytics: { trackCurrentState: true, trackChanges: true, valueKind: 'enum', privacy: 'safe', identityScope: 'person' },
    },
    filesMarkdownRichEditorMaxBytes: {
        schema: z.number(),
        default: 256_000,
        description: 'Maximum markdown file size (bytes) for which the rich (WYSIWYG) editor is offered before falling back to raw',
        storageScope: 'account',
        analytics: {
            trackCurrentState: true,
            trackChanges: true,
            ...
            privacy: 'bucketed',
            identityScope: 'person',
            serializeCurrent: serializeBytesBucket(256_000, 512_000),
        },
    },
    filesMarkdownRichEditorHtmlRoundTripMaxBytes: {
        schema: z.number(),
        default: 50_000,
        description: 'Maximum markdown body size (bytes) to attempt the web HTML round-trip eligibility check before conservatively falling back to raw',
        storageScope: 'account',
        analytics: {
            trackCurrentState: true,
            trackChanges: true,
            ...
            privacy: 'bucketed',
            identityScope: 'person',
            serializeCurrent: serializeBytesBucket(50_000, 100_000),
        },
    },
