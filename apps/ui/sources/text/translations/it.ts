  ...
  files: {
    ...
    fileEditor: {
      ...
      loadingFile: ({ fileName }: { fileName: string }) =>
        ...
        mediaPreviewTooLarge: 'Il file multimediale o documento è troppo grande per l’anteprima nell’app',
        legacyOfficeFormat: 'Il formato Office legacy non è supportato per l’anteprima in linea. Converti in un formato moderno (.docx, .xlsx, .pptx).',
        pdfPreviewFailed: 'Impossibile caricare l’anteprima del PDF',
        officePreviewFailed: 'Impossibile visualizzare l’anteprima del documento Office',
        audioPlaybackFailed: 'Riproduzione audio non riuscita',
        videoPlaybackFailed: 'Riproduzione video non riuscita',
        play: 'Riproduci',
        pause: 'Pausa',
        pptxSlide: ({ current, total }: { current: number; total: number }) => `Diapositiva ${current} di ${total}`,
        pptxEmpty: 'Nessun contenuto di diapositiva trovato nella presentazione',
        xlsxSheet: ({ name }: { name: string }) => `Foglio: ${name}`,
        diff: "Differenze",
      file: "Documento",
      markdown: "Markdown",
    diffModes: {
