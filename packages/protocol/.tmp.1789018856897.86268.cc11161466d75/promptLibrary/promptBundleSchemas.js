...
function normalizePosixRelativePath(raw) {
    ...
    if (value.includes('\\'))
        return { ok: false, message: 'path must use posix separators' };
    if (value.includes('\u0000'))
        return { ok: false, message: 'path contains null byte' };
    const segments = value.split('/');
    if (segments.some((s) => s.length === 0))
        return { ok: false, message: 'path contains empty segment' };
    for (const seg of segments) {
        if (seg === '.' || seg === '..')
            return { ok: false, message: 'path contains dot segment' };
    }
    return { ok: true, normalized: segments.join('/') };
}
export function validatePromptBundleBodyV1AgainstSchemaId(input) {
...
//# sourceMappingURL=promptBundleSchemas.js.map